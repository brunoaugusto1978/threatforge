...
def hash_secret(secret_value: str) -> str:
    """Hashes high-entropy machine secrets without human password policy checks."""
    if _ARGON2:
        return _ph.hash(secret_value)
    return _pbkdf2_hash(secret_value)


def generate_api_key() -> tuple[str, str, str]:
    """Gera uma API key de tenant. Retorna (chave_completa, prefix, secret_hash).

    The full key is displayed only once; the database stores prefix + slow hash.
    Existing SHA-256/HMAC API keys must be regenerated after this hardening.
    """
    secret = secrets.token_urlsafe(32)
    full = f"tfk_{secret}"
    prefix = full[:12]
    digest = hash_secret(full)
    return full, prefix, digest


def verify_api_key(api_key: str, stored: str) -> bool:
    """Verifies a tenant API key against its stored slow hash."""
    return verify_password(api_key, stored)
